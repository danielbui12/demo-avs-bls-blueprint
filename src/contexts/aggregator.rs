...
use crate::{
    ...
    contexts::eigen_task::{IndexedTask, TangleTaskResponseSender},
};
...
use blueprint_sdk::tokio::sync::{Mutex, Notify, oneshot};
...
pub struct AggregatorContext {
    ...
    pub task_aggregator:
        Option<Arc<TaskAggregator<IndexedTask, TaskResponse, TangleTaskResponseSender>>>,
}
...
impl AggregatorContext {
    pub async fn new(
        ...
        let response_sender = TangleTaskResponseSender {
            task_manager_address,
            http_rpc_url: env.http_rpc_endpoint.clone(),
        };
    ...
    pub async fn start(self) -> JoinHandle<()> {
        ...
        blueprint_sdk::tokio::spawn(async move {
            ...
            let server_handle = blueprint_sdk::tokio::spawn(Self::start_server(Arc::clone(&aggregator)));
    ...
    pub async fn shutdown(&self) {
        ...
        if let Some(task_agg) = &self.task_aggregator {
            match blueprint_sdk::tokio::time::timeout(Duration::from_secs(10), task_agg.stop()).await {
    ...
    async fn start_server(aggregator: Arc<Mutex<Self>>) -> Result<(), Error> {
            ...
            move |params: Params| {
                ...
                async move {
                    ...
                    let signed_task_response: SignedTaskResponse =
                        jsonrpc_core::serde_json::from_value(inner_params.clone()).map_err(|e| {
        ...
        let server_handle = blueprint_sdk::tokio::task::spawn_blocking(move || {
            server.wait();
